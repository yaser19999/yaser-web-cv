...
export const MAX_FILE_SIZE = 30 * 1024 * 1024 // 25MB
...
export const handleImageUpload = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (file.size > MAX_FILE_SIZE)
      ...
      return reject(
        new Error(
          `HEIC images is not supported`
        )
      )


    // ✅ THE RIGHT WAY: Create a temporary Blob URL
    // This returns a string like "blob:http://localhost:3000/uuid..."
    const blobUrl = URL.createObjectURL(file);


    // Resolve immediately with the blob URL (no waiting for reader!)
    resolve(blobUrl);
    ...
    //   resolve(e.target?.result as string)  // ✅ base64 string here
    // }

    // reader.readAsDataURL(file) // ❌ returns void (but kicks off async operation)
